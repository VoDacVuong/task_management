...
class JWTAuthentication(authentication.BaseAuthentication):
    ...
    def _authenticate_credentials(self, request, token):
        ...
        # if not user.active or user.status != enums.UserStatus.AVAILABLE:
        #     raise exceptions.AuthenticationException(messages.CONTACT_ADMIN_FOR_SUPPORT)
        # check_device_token(token)
        return (user, token)

def check_device_token(token):
    from oauth.models import UserDeviceToken
    user_device_token = UserDeviceToken.get_by_token(token)

    if not user_device_token or not user_device_token.active:
        raise exceptions.AuthenticationException(messages.INVALID_OR_EXPIRED_TOKEN)
    elif (user_device_token.created_at + timedelta(days=settings.TOKEN_LIFETIME)) < datetime.now():
        raise exceptions.AuthenticationException(messages.INVALID_OR_EXPIRED_TOKEN)
